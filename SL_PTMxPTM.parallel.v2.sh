...
#SBATCH --error=PTMxPTMcross.%J.err
#SBATCH --output=PTMxPTMcross.%J.out
...
Raw_reads=${main_dir/FGOB10Ptm-1_X_SG1_ptm/}/raw_reads/
...
#####map to PTM
for each in ${entry}_SG1.fq
do
isolate=${each%_SG1.fq}
echo $isolate

${bwa_soft}bwa mem -M -t 4 ${refPTM} \
${isolate}_SG1.fq > ${bwa_out}${isolate}.PTM.SE.sam
done


#####map to PTT
for each in ${entry}_PTM_FGOB10Ptm-1_assembly.v7.fq
do
isolate=${each%_PTM_FGOB10Ptm-1_assembly.v7.fq}
echo $isolate

${bwa_soft}bwa mem -M -t 4 ${refPTT} \
${isolate}_PTM_FGOB10Ptm-1_assembly.v7.fq > ${bwa_out}${isolate}.PTT.SE.sam
...
grep -v "OCTF" ${entry}.show-coords|grep "chrPTM.*chrPTM\|mito.*mito" |awk '{print $12 "\t" $1 "\t" $2 "\t" $13 ":" $4 "-" $5}' |bedtools sort -i stdin -faidx ${refPTT}.fai > ${entry}.PTT.bed
grep  "OCTF" ${entry}.show-coords|awk '{print $12 "\t" $1 "\t" $2 "\t" $13 ":" $4 "-" $5}' |bedtools sort -i stdin -faidx ${refPTT}.fai>${entry}.PTM.bed

bedtools intersect -a ${entry}.PTT.bed -b ${entry}.PTM.bed -nonamecheck|bedtools sort -i stdin -faidx ${refPTT}.fai \
|bedtools merge -i stdin|awk   '{print $0 "\t" "common"}'> ${entry}.common.bed
bedtools subtract -a ${entry}.PTM.bed -b ${entry}.PTT.bed -nonamecheck|cut -f1,2,3 \
|bedtools sort -i stdin -faidx ${refPTT}.fai|bedtools merge -i stdin|awk   '{print $0 "\t" "PTM"}'> ${entry}.PTMuniq.bed
bedtools subtract -a ${entry}.PTT.bed -b ${entry}.PTM.bed -nonamecheck|bedtools sort -i stdin -faidx ${refPTT}.fai \
|bedtools merge -i stdin|awk   '{print $0 "\t" "PTT"}'> ${entry}.PTTuniq.bed
...
awk -v OFS='\t' '{print $14,$1,$1,$15}' ${entry}.snps |grep "O" |grep -v "home"|bedtools sort -i stdin -faidx ${refPTT}.fai > ${entry}.PTM.snps
...
| awk '{print $0 "\t" $3-$2}' | awk -v OFS="\t" '$5>=1000{print $1,$2,$3,$4}'> ${entry}.common_unique.blocks.bed
