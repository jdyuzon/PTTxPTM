...
def mktest(codon_alns, codon_table=None, alpha=0.05):
    ...
    for i in codon_set:
        ...
        if fix_or_not:
            ...
            syn_poly += this_syn
    ###returns ds,dn,ps,pn; dn/ds; pn/ps; NI;alpha;DoS (JDY)
    try: 
        dnds=(nonsyn_fix/syn_fix)
    except (ZeroDivisionError,ValueError) as err: 
        dnds="NA"
    try:
        pnps=(nonsyn_poly/syn_poly)
    except (ZeroDivisionError,ValueError) as err:
        pnps="NA"
    try:
        NI=((nonsyn_poly*syn_fix)/(syn_poly*nonsyn_fix))
    except (ZeroDivisionError,ValueError) as err:
        NI="NA"
    try:
        a=(1-(nonsyn_poly*syn_fix)/(syn_poly*nonsyn_fix))
    except (ZeroDivisionError,ValueError) as err:
        a="NA"
    try:
        DoS=( (nonsyn_fix/(nonsyn_fix+syn_fix)) - (nonsyn_poly/(nonsyn_poly+syn_poly)) )
    except (ZeroDivisionError,ValueError) as err:
        DoS="NA"
    return (syn_fix, nonsyn_fix, syn_poly, nonsyn_poly,dnds,pnps,NI,a,DoS)
