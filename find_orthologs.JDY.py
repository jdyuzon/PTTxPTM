...
if sys.version_info[0] < 3:
    ...
    sys.exit(1)
import argparse
import textwrap, random, tempfile, os, subprocess, statistics, re
...
# Make sure the query sequences contains terminal stop codons but no premature stop codons. (JDY)
for x in range(len(query_seqs)):
    if query_seqs[x][-1] == "*":
        query_seqs[x] = query_seqs[x]
    if "*" in query_seqs[x][:-2]:
...
for z in range(len(query_window_ids)):
    ...
    for subject in subjects:
        ...
        if blast_lines == [""]:
            ...
            # If tblastn was used, remove all hits that contain an internal stop codon (*) and don't have a start codon (JDY).
            if translate:
                tmp_qseqids = []
                tmp_sseqids = []
                tmp_evalues = []
                ...
                tmp_sseqs = []
                for x in range(len(qseqids)):
                    if sseqs[x][0]=="M" and "*" not in sseqs[x][:-2]:
                        tmp_qseqids.append(qseqids[x])
                        tmp_sseqids.append(sseqids[x])
                        tmp_evalues.append(evalues[x])
                        tmp_bitscores.append(bitscores[x])
    ...
    if translate and write_this_alignment:
        ...
        for x in range(len(nucl_subject_seq_per_subject)):
            if strictness == 0:
                ...
                # Make sure that the nucleotide sequence contains three times the number of sites
                # of the amino acid sequence.
                # added "3" to amino acid to account for stop codon (JDY)
                if ((len(nucl_subject_seq_per_subject[x]) != 3 * len(aln_seqs[aln_index].replace("-","")) + 3)):
                #if len(nucl_subject_seq_per_subject[x][y]) != 3 * len(aln_seqs[aln_index].replace("-","")):
                    print("ERROR: The length of the amino acid and nucleotide sequcences don't match even without the terminal stop codon!")
                    print("ERROR: " + aln_ids[aln_index])
                    print("ERROR: " + nucl_subject_seq_per_subject[x])
                    print("ERROR: " + aln_seqs[aln_index])
                    print("AA length: " + str(3 * len(aln_seqs[aln_index].replace("-",""))))
                    print("nt length: " + str(len(nucl_subject_seq_per_subject[x])) )
                    # sys.exit(1) (JDY)
                nucl_pos = 0
                # decreased Amino Acid length by - 1 to account for stop codon (JDY)
                # print(range(len(aln_seqs[x+1]) - 1))
                for aa_pos in range(len(aln_seqs[x+1])):
                    # print("TESTING aa_pos " + str(aa_pos))
                    if aln_seqs[x+1][aa_pos] == "-":
                        aligned_nucl_subject_seq_per_subject += "---"
                    else:
                        aligned_nucl_subject_seq_per_subject += nucl_subject_seq_per_subject[x][nucl_pos:nucl_pos+3]
                        nucl_pos += 3
                # add stop codon/last three nt from original nucleotide sequence (JDY)
                aligned_nucl_subject_seq_per_subject+=nucl_subject_seq_per_subject[x][-3:]
                # print(aligned_nucl_subject_seq_per_subject)
                untranslated_aln_seqs.append(aligned_nucl_subject_seq_per_subject)
        ...
        if len(untranslated_aln_seqs) > 0:
            ...
            # decreased nucleotide sequence by - 3 to account for terminal stop codon/last three nucleotides (JDY)
            for x in range(len(untranslated_aln_seqs[0]) - 3):
                gap_only_site = True
                for untranslated_aln_seq in untranslated_aln_seqs:
                ...
                if gap_only_site == False:
                    for y in range(len(untranslated_aln_seqs)):
                        #print("range " + str(range(len(untranslated_aln_seqs))))
                        print(str(y) + " " + untranslated_aln_seqs[y][x])
                        tmp_untranslated_aln_seqs[y] += untranslated_aln_seqs[y][x]
            untranslated_aln_seqs = tmp_untranslated_aln_seqs
        print(" done with Remove gap-only sites.")
            ...
            if translate:
                if len(untranslated_aln_seqs) > 0:
                    for untranslated_aln_count in range(len(untranslated_aln_ids)):
                        ...
                        untranslated_aln_string += "\n"
                        # add stop codon/last three nt from original nucleotide sequence (JDY)
                        untranslated_aln_seqs[untranslated_aln_count]+=nucl_subject_seq_per_subject[untranslated_aln_count][-3:]
                        print(untranslated_aln_seqs[untranslated_aln_count])
                        untranslated_aln_string += untranslated_aln_seqs[untranslated_aln_count]
                        untranslated_aln_string += "\n"
